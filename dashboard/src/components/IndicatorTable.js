...
function getFreshnessColor(isStale, seconds) {
    if (isStale)
        return "#ef4444"; // red
    if (seconds > 60 * 60)
        return "#f59e0b"; // amber — over 1h
    if (seconds > 30 * 60)
        return "#eab308"; // yellow — over 30m
    return "#10b981"; // green
}
function DataFreshnessCell({ indicator, isProfessional, }) {
    ...
    if (ageSeconds === null) {
        return _jsx("span", { style: { color: C.textMuted, fontSize: 12 }, children: "\u2014" });
    }
    ...
    const tooltip = isoTimestamp
        ...
        : `Age: ${relative}`;
    return (_jsxs("div", { style: { display: "flex", alignItems: "center", gap: 5 }, title: tooltip, children: [_jsx("div", { style: {
                    width: 6,
                    height: 6,
                    borderRadius: "50%",
                    background: color,
                ...
                } }), _jsx("span", { style: { color, fontSize: 12, fontVariantNumeric: "tabular-nums" }, children: isProfessional && isoTimestamp
                    ? new Date(lv.recordedAt).toLocaleTimeString()
                    : relative }), isStale && (_jsx("span", { title: "Data is stale", style: {
                    fontSize: 10,
                    color: "#ef4444",
                    background: "#ef444422",
