...
type ResultType = "page" | "indicator" | "signal" | "command";
...
const PAGE_RESULTS: Omit<SearchResult, "action">[] = [
  ...
  function handleCommandAction(id: string) {
    switch (id) {
      ...
      case "cmd-backtest-2022":
        ...
        break;
      case "cmd-sound-on":
        try { localStorage.setItem("mp_sound_alerts", "true"); } catch {}
        onClose();
        break;
      case "cmd-sound-off":
        try { localStorage.setItem("mp_sound_alerts", "false"); } catch {}
        onClose();
        break;
      case "cmd-refresh":
  ...
  const buildResults = useCallback((): SearchResult[] => {
    ...
    if (isCommand) {
      return COMMANDS.filter(
        (c) => !searchTerm || c.title.toLowerCase().includes(searchTerm) || (c.subtitle ?? "").toLowerCase().includes(searchTerm)
      ).map((c) => ({
        ...c,
      ...
      >
        ...
        >
          ...
          <input
            ...
            placeholder="Search pages, indicators, signals… or type > for commands"
            style={{
              flex: 1,
        ...
        {!query && recentSearches.length > 0 && (
          <div style={{ padding: "8px 12px 4px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ color: C.textMuted, fontSize: 10, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 6 }}>
              Recent
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ color: C.textPrimary, fontSize: 13, fontWeight: isSelected ? 600 : 400 }}>
                      {result.title}
                    </div>
                    {result.subtitle && (
        ...
        >
          <span style={{ color: C.textMuted, fontSize: 11 }}>↑↓ Navigate · Enter Select · ESC Close</span>
          <span style={{ color: C.textMuted, fontSize: 11 }}>Type <strong style={{ color: C.textSecondary }}>&gt;</strong> for commands</span>
        </div>
      </div>
    </>
  );
